...
/* round precisely to decimalPoint*/ 
...
const findTermInYearsMonths = (bal, rate, payment) => {
  ...
  const term = findTerm(bal, rate, payment);
  const termInYears = term/12;
  const years = Math.floor(termInYears); // get integer part of number
  const months = round(termInYears-years,2)/100; // get decimal part & convert to integer
